"""GET /monitoring/logs 整合測試（monitoring.md §7.3）。"""
...
async def test_logs_no_auth_returns_401(client: AsyncClient) -> None:
    resp = await client.get("/monitoring/logs")
...
async def test_logs_empty_returns_200(
    ...
    resp = await client.get("/monitoring/logs", headers=_auth_header(token))
...
async def test_logs_filter_by_level(
    ...
    token = await _admin_token(client)
    resp = await client.get("/monitoring/logs?level=ERROR", headers=_auth_header(token))
    assert resp.status_code == status.HTTP_200_OK
    data = resp.json()
...
async def test_logs_limit_and_cursor_pagination(
    ...
    token = await _admin_token(client)
    resp1 = await client.get("/monitoring/logs?limit=4", headers=_auth_header(token))
    ...
    resp2 = await client.get(
        f"/monitoring/logs?limit=4&cursor={data1['next_cursor']}",
...
async def test_logs_limit_clamped_to_max(
    ...
    token = await _admin_token(client)
    resp = await client.get("/monitoring/logs?limit=99999", headers=_auth_header(token))
    assert resp.status_code == status.HTTP_200_OK
