"""Admin WebSocket 端點（websocket §2.1/§3.1）。

- POST /admin/ws/ticket：HTTP + JWT 認證（get_current_admin）→ 換一張短命單次 ticket。
- GET  /admin/ws：WebSocket upgrade，帶 ticket；accept 前 GETDEL 驗票 → 重載 Admin 讀現值
...
router: APIRouter = APIRouter(prefix="/admin", tags=["admin-ws"])
