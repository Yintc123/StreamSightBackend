...
> **⚠️ 時區陷阱（務必遵守）**：欄位雖宣告 `DateTime(timezone=True)`，但在 **SQLite（測試環境）讀回來的 `datetime` 是 naive（無 `tzinfo`）**——SQLite 無原生時區。本模組是全專案第一次對「DB 讀回的 datetime」做算術（`now - revoked_at`、`expires_at <= now`），既有 `created_at/updated_at` 從未被相減故未暴露此問題。**所有拿 `expires_at` / `revoked_at` 與 `now`（aware）比較前，一律先正規化為 aware-UTC**：
> ```python
> def _as_utc(dt: datetime) -> datetime:
>     return dt if dt.tzinfo else dt.replace(tzinfo=UTC)
> ```
> 漏了這步，`aware - naive` 會直接 `TypeError`，且**單元/整合測試（跑 SQLite）一開跑就紅**。建議把 `_as_utc` 放在 service 模組層供 `refresh` 重用。
...
- `revoke_all_for_user(user_id: int, revoked_at: datetime) -> int`：撤銷該 user 所有 `revoked_at IS NULL` 的列，回傳撤銷筆數（登出所有裝置用）。
  > jwt-role 已將擁有者改為 principal → 此方法對應為 `revoke_all_for_principal(principal_id, revoked_at)`（見文首 ⚠️）。
- `has_live_tokens_in_family(family_id: str, *, now: datetime) -> bool`：查該 family 是否仍有「**未撤銷且未過期**」的 token，即 `SELECT EXISTS(... WHERE family_id=:fid AND revoked_at IS NULL AND expires_at > :now)`。**唯讀、不寫入**；供 **WebSocket 定期複查 session 有效性**用（[`websocket.md`](./websocket.md) §2.2／§2.5：`logout` 不改 `is_active`，靠此判斷整個 session 是否已登出 → 若無 live token 則斷該 sid 的 WS，`close(4401)`）。
  > **為何要 `expires_at > now`**：只查 `revoked_at IS NULL` 會把「未撤銷但已自然過期」的殘列誤判為 session 仍活；WS 複查須以「還有可用 token」為準，故一併排除過期列。走 `family_id` 既有 index，`EXISTS` 早停、成本可忽略（admin 連線數少、每複查週期一次）。
...
- `refresh(payload: RefreshRequest) -> TokenPayload`：`now = datetime.now(UTC)`
  ...
  3. **已撤銷（`rt.revoked_at` 非 NULL）→ reuse 判定（含 grace，見 2.6）**（比較前先 `_as_utc(rt.revoked_at)`，見 §3 時區陷阱）：
     - 若 `now - _as_utc(rt.revoked_at) <= timedelta(seconds=grace_seconds)`（剛輪替的良性並發/重試）→ `raise UnauthorizedError("Invalid refresh token")`（**只 401、不撤 family、無寫入**）。
  ...
  4. 已過期（`_as_utc(rt.expires_at) <= now`）→ `raise UnauthorizedError("Refresh token expired")`（無寫入）。
...
> 時間一律 `now = datetime.now(UTC)`，沿用 `app/core/auth/jwt.py` 慣例；**與 DB 讀回的 datetime 比較前務必 `_as_utc()` 正規化（見 §3 時區陷阱）**。`grace_seconds` 讀自 `settings.refresh_token_reuse_grace_seconds`。錯誤訊息刻意統一、模糊化，延續 `login` 防列舉（user enumeration）的既有做法。
...
- 既有 `register` / `login` 回傳的 `TokenResponse` 一併填 `expires_in=settings.jwt_access_token_expire_seconds`（建議抽一個小 helper `_to_token_response(token: TokenPayload) -> TokenResponse` 統一組裝，避免各端點重複）。
  - **⚠️ 必改既有呼叫點**：目前 `register` / `login` 是 `TokenResponse(**token.model_dump())`。`expires_in` 若設為**必填**（無預設），`token.model_dump()`（來自 `TokenPayload`，不含 `expires_in`）會導致驗證失敗。故**這兩個既有呼叫點必須改用 helper**（或給 `expires_in` 一個預設值）。這也會連帶動到既有整合測試的期待（新增欄位，非精確鍵比對故不破壞）。
...
- `delete_expired` **刪除一條 OLD→NEW（`replaced_by_id` 互參考）皆過期的輪替鏈**不報 FK 違規（驗證自參考 FK + `SET NULL` 在 `PRAGMA foreign_keys=ON` 下的批次刪除，見 §5.2）。
- **`has_live_tokens_in_family`（WS 複查用，見 §5.2）**：
  - family 有一筆 active（未撤、未過期）token → `True`；同 family 另建已撤銷列不影響結果仍 `True`。
  - family 全部 token 皆已 `revoked_at`（模擬 `logout` / `logout_all`）→ `False`。
  - family 僅剩「未撤銷但 `expires_at <= now`」的過期列 → **`False`**（驗證一併排除過期列，非只看 `revoked_at`）。
  - 不存在的 `family_id` → `False`。
...
- `refresh(已撤銷、在 grace 內)` → **只拋 `UnauthorizedError`、family 其餘 token 不受影響**（grace 良性路徑，見 2.6）。以直接寫入 `revoked_at = now`（grace 內）建構。
  > 註：此案例與「已過期」案例會實際執行 `now - revoked_at` / `expires_at <= now` 的時間算術，因此在 SQLite 下天然是 `_as_utc` 正規化（§3 時區陷阱）的 RED 測試——未正規化會 `TypeError`。
...
4. **repository**：`RefreshTokenRepository`（`get_by_hash` / `consume` 原子 / `revoke_family` / `revoke_all_for_user` / `delete_expired`）+ `repositories/__init__` 匯出（8.2）。bulk 語句一律 `synchronize_session=False`；`delete_expired` 需過自參考 FK 鏈測試（見 §5.2）。
5. **dto/schema**：`TokenPayload` 加 `refresh_token`、`RefreshRequest` + `dtos/__init__` 匯出、`TokenResponse` 加 `expires_in`。
6. **service**：`_as_utc` 時區正規化 helper（§3）、`_issue_refresh_token` / `register`+`login`（含 opportunistic 清理）發 refresh / `refresh`（原子消費 + grace + commit-before-raise）/ `logout` / `logout_all`（8.3）。
7. **router**：`register`/`login` 改用 helper 填 `expires_in`（必改既有呼叫點，§5.6）、`/auth/refresh`、`/auth/logout`、`/auth/logout-all`（8.4）。
