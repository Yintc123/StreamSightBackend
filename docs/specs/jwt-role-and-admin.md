...
- **新增** `app/api/routers/admin/`（目前空 placeholder）：
  - `POST /admin/auth/login` → admin login → `200` `TokenResponse`（role 1 的 access + refresh + expires_in）。初始 admin（哨兵 `principal_id=0`）例外：無 refresh、`expires_in` 為 10800（3 小時）。
...
- `POST /admin/auth/login` 正確（DB admin）→ `200`（含 refresh + expires_in = 全域預設值）；初始 admin → `200`（refresh=null、expires_in=10800）；錯密碼 → `401`。
