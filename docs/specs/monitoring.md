...
| 資料庫狀態監控 | MariaDB status/PROCESSLIST + SQLAlchemy pool（sampler 週期採） | `monitor.db`（sampler 推） | `GET /admin/monitoring/db`（當下快照）＋歷史（走歷史查詢） | `monitor:stream:db` |
...
**端點（暫定，掛既有 `/admin` 前綴，全走 `get_current_admin` + topic/端點級 `require_min_admin_role`）**：
`GET /admin/monitoring/logs`、`GET /admin/monitoring/db`、`GET /admin/monitoring/metrics/{name}`。
...
- **背景 flush task**（lifespan 啟動）：每 `flush_interval` 或滿 `flush_batch_size` → 批次 `XADD` 進 `monitor:stream:logs`；失敗只 log、佇列繼續（best-effort）。
...
- **可選即時 tail**：flush 時對 `monitor.logs` topic 同步 `Publisher.to_topic`（訂閱者才收；預設關，避免高頻 log 灌爆 WS，走 config）。
...
- **`MonitoringSampler`**：每 `db_sample_interval` 採一次 DB 狀態 → **同一份樣本**：`Store.append("monitor:stream:db", sample)` **且** `Publisher.to_topic("monitor.db", event(sample))`（落地 + 即時一次到位）。
...
| `backend` | str | `mariadb`/`sqlite`（probe 能力標示） |
...
### 3.4 WS 即時封套（重用 websocket §3.3 `event`）
...
class MonitoringSampler:
    def __init__(self, client: redis.Redis, probe: DbStatsProbe,
                 store: TimeSeriesStore, publisher: Publisher) -> None: ...
...
| `monitoring_db_sample_interval_seconds` | `15` | 秒 | §2.5 | DB 狀態採樣週期 |
| `monitoring_db_stream_maxlen` | `10000` | 則 | §2.9 | `monitor:stream:db` 近似上限 |
| `monitoring_sampler_leader_lease_seconds` | `30` | 秒 | §2.5 | leader lease；須 `≥ 2×` 採樣週期 |
| `monitoring_query_max_limit` | `500` | 則 | §2.7 | 查詢單頁上限（防重負載） |
| `monitoring_retention_seconds` | `604800`（7d） | 秒 | §2.9 | 可選按時間修剪（`MINID`）；`0` = 只靠 `MAXLEN` |
...
日誌擷取（非阻塞）：
  app logging ─ RedisStreamLogHandler.emit() ─ put_nowait(有界佇列) ─(滿→丟最舊)
    背景 flush task：每 1s / 滿 100 則 → XADD monitor:stream:logs MAXLEN ~ 10000
                                       └(可選) Publisher.to_topic("monitor.logs", event)

DB 狀態採樣（多實例單 leader）：
  每 15s：SET monitor:sampler:leader NX EX 30 ─ 搶到? ─否→ 待命
    └是→ probe.sample()（短命 session 讀 SHOW GLOBAL STATUS / PROCESSLIST + pool）
          ├ Store.append("monitor:stream:db", sample) MAXLEN ~
...
- `run_log_flusher` 一輪 → 批次 `XADD` 進 stream；遮罩 filter 生效（email 已遮）。
- flush 遇 Redis 例外 → 只 log、佇列續存（不中斷）。
...
- `GET /admin/monitoring/metrics/db?since=&until=` → range 回放、游標分頁；未知 `name` → 404/422。
...
2. `TimeSeriesStore` 介面 + `RedisStreamStore`（`append`/`query`）（7.1）。
...
8. topic 授權（`TOPIC_MIN_ROLE["monitor.db"/"monitor.logs"]`）+ 資源上限 + 非關鍵路徑隔離（7.6/7.8、§6）。
9. 提交前檢查全綠（ruff / ruff format / pyright / pytest）；真 Redis + 真 MariaDB（`SHOW GLOBAL STATUS`／`PROCESSLIST`）煙霧測試。
...
- ✅ **日誌擷取非阻塞**：`logging.Handler` → 有界佇列（滿即丟）→ 背景 flush 批次 `XADD`；沿用既有遮罩 filter；`APP_ENV=test`/`monitoring_enabled=False` 不啟用。
