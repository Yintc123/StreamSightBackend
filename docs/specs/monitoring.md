...
|---|---|---|---|---|
| 系統日誌查詢 | app logging（非阻塞 handler） | `monitor.logs`（可選 tail） | `GET /admin/monitoring/logs`（篩選+游標分頁） | `monitor:stream:logs` |
| 資料庫狀態監控 | MariaDB status/PROCESSLIST + SQLAlchemy pool（sampler 週期採） | `monitor.db`（sampler 推） | `GET /admin/monitoring/db`（當下快照）、`GET /admin/monitoring/db/history`（股價式折線圖，ZADD Sorted Set） | `monitor:stream:db`（Stream）＋ `monitoring:db:history`（Sorted Set） |
| 即時資料歷史查詢 | 上述 sampler 樣本（同一份） | 重用上面兩個 topic | `GET /admin/monitoring/metrics/{name}`（range 查詢） | `monitor:stream:{name}` |
...
**端點（掛既有 `/admin` 前綴，全走 `get_current_admin` + topic/端點級 `require_min_admin_role`）**：
`GET /admin/monitoring/logs`、`GET /admin/monitoring/db`、`GET /admin/monitoring/db/history`、`GET /admin/monitoring/metrics/{name}`。

> 🔗 **跨規格端點**：`GET /admin/monitoring/infra`（OS 硬體層 + MariaDB 引擎層指標，Sorted Set 歷史查詢）定義於 [`infra-monitoring.md`](./infra-monitoring.md)，與本模組共用同一 router 檔案（`app/api/routers/admin/monitoring.py`）但屬不同採集骨架（InfraSampler 輪詢 exporter，非 MonitoringSampler）。`GET /health/node-exporter`、`GET /health/mysqld-exporter`（exporter 可達性 health check）定義於 [`infra-monitoring.md §5`](./infra-monitoring.md)。
...
- **`MonitoringSampler`**：每 `db_sample_interval`（預設 5 秒）採一次 DB 狀態 → **同一份樣本三路輸出**：
  ...
  3. `redis.zadd(monitoring_db_sorted_set_key, {json.dumps(sample): ts_ms})` + `redis.zremrangebyscore(…, 0, ts_ms - retention_ms)` — 落地至 **Redis Sorted Set**（score＝epoch ms）供 `GET /admin/monitoring/db/history` 時間區間查詢（股價式折線圖）；同步以 `ZREMRANGEBYSCORE` 執行時間保留（預設 24h），無需 `MAXLEN`。
...
- **日誌**：`GET /admin/monitoring/logs?level=&since=&until=&request_id=&logger=&cursor=&limit=`
...
- **DB 快照**：`GET /admin/monitoring/db` → 即時採一次（或回最近一筆 stream entry）回當下狀態。
- **DB 歷史（股價式折線圖）**：`GET /admin/monitoring/db/history?start_ms=&end_ms=`
...
- **歷史指標**：`GET /admin/monitoring/metrics/{name}?since=&until=&cursor=&limit=`（`name` ∈ 允許集合，如 `db`）→ range 回放。
...
app/
├── api/routers/admin/
│   └── monitoring.py            # GET logs / db / metrics（認證 + require_min_admin_role）
├── services/monitoring/
...
### 3.3 `DbHistoryResponse`（`GET /admin/monitoring/db/history` 回應）
...
```jsonc
// GET /admin/monitoring/logs?level=ERROR&limit=50
{
...
- **端點授權**：`GET /admin/monitoring/*` 以 `Depends(get_current_admin)` + 視敏感度加 `Depends(require_min_admin_role(...))`（logs 建議 `SUPER_ADMIN`）。
- **`GET /admin/monitoring/db/history`**：直接 `Depends(get_redis)`（無需 service layer，僅讀 Sorted Set）；回 `DbHistoryResponse(snapshots=…)`。
...
>
> ⚠️ **跨規格依賴**：`GET /admin/monitoring/db/history` 的「僅帶 end_ms / 無參數」情境使用 `monitoring_infra_default_query_hours`（定義於 `infra-monitoring.md §4.3`，預設 `1` 小時）作為預設查詢窗口，未在本表另行定義。此為刻意共享（兩個 Sorted Set API 語意完全對稱），修改此值將同時影響 `/db/history` 與 `/infra`。
...
前端標準用法（股價式折線圖）：
  開頁：GET /admin/monitoring/db/history?start_ms=now-1h（拉近 1 小時歷史 snapshots）
      → WS subscribe "monitor.db"（每 5 秒接收增量 event，append 到圖表）

查詢（HTTP，游標分頁）：
  GET /admin/monitoring/logs?level=ERROR&since=&cursor=&limit=50
...
- `GET /admin/monitoring/db`（注入假 probe）→ 200 + 快照結構；非 admin 拒絕。
...
- `GET /admin/monitoring/metrics/db?since=&until=` → range 回放、游標分頁；未知 `name` → **200 + 空 items**（對應 Redis Stream 上無資料；Stream 查詢語意為「空區間回空頁」，不視為請求錯誤，由呼叫端依業務決定是否提示）。
...
4. `LogQueryService` + `GET /admin/monitoring/logs`（授權 + 游標分頁）（7.3）。
5. `DbStatsProbe`（MariaDb/Pool/退化）+ `DbStatsService` + `GET /admin/monitoring/db`（7.4）。
6. `MonitoringSampler`（leader lease + 採樣 → 落地 + 推播）+ lifespan 啟停（7.5/7.6）。
7. `MetricQueryService` + `GET /admin/monitoring/metrics/{name}`（歷史 range）（7.7）。
8. topic 授權（`TOPIC_MIN_ROLE["monitor.db"/"monitor.logs"]`）+ 資源上限 + 非關鍵路徑隔離（7.6/7.9、§6）。
9. **MonitoringSampler Sorted Set 雙寫**（`sorted_set_key`/`retention_hours`）+ `GET /admin/monitoring/db/history`（`DbHistoryResponse`，ZRANGEBYSCORE）（7.5 Sorted Set 測試 + 7.8）。
...
- ✅ **MonitoringSampler 雙寫（Stream ＋ Sorted Set）**：Stream（`XADD MAXLEN ~`）供 WS tail / 游標分頁；Sorted Set（`ZADD score=epoch_ms` + `ZREMRANGEBYSCORE` 時間保留）供 `GET /admin/monitoring/db/history` 股價式折線圖查詢（`ZRANGEBYSCORE start_ms end_ms`）。兩者職責不同、共用同一份採樣，採樣間隔從 15 秒縮短至 **5 秒**，Sorted Set 保留 24 小時。
- ✅ **`GET /admin/monitoring/db/history`**：4 種時間參數情境（帶兩者/只 start/只 end/無參數）與 `/infra` 語意一致；`start >= end` → 400；範圍超出 `monitoring_db_retention_hours` → 400；Redis 不可用 → 503；回 `DbHistoryResponse(snapshots=list[DbSample])`（oldest-first）。
