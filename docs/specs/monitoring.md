...
> 狀態：**已實作（✅ 625 tests 全綠，ruff / pyright 通過）** ／ 目標版本：next+4 ／ 開發模式：**嚴格 TDD（見 `CLAUDE.md`）**
>
> **語言**：繁體中文。
>
...
- **有界 hot buffer，非長期歸檔**：每 stream 設 `MAXLEN ~ N`（近似修剪，`~` 讓 Redis 批次刪除更省）；**✅ 已實作**：`MINID ~` 按時間修剪（保留近 `monitoring_retention_seconds`，預設 7 天）。兩者可共存，各自從不同維度淘汰最舊筆；`minid_seconds=0` 則只靠 `MAXLEN`（向後相容）。逾量最舊者淘汰——監控是 best-effort，不保證無限歷史。
- **Store 介面（Protocol）解耦後端**：
  ```python
  class TimeSeriesStore(Protocol):
      async def append(self, stream: str, entry: dict, *,
                       maxlen: int | None = None, minid: int | None = None) -> str: ...          # 回 entry id
      async def append_many(self, stream: str, entries: list[dict], *,
                            maxlen: int | None = None, minid: int | None = None) -> list[str]: ...  # pipeline 批次，回 id list
      async def query(self, stream: str, *, since: int | None, until: int | None,
                      cursor: str | None, limit: int) -> Page: ...                               # 時間/ID 區間 + 游標
  ```
  - `approximate=True` 在 `maxlen is not None or minid is not None` 時自動啟用（`MAXLEN ~` 與 `MINID ~`）；兩者皆 None 時 `approximate=False`。
...
- **背景 flush task**（lifespan 啟動）：每 `flush_interval` 或滿 `flush_batch_size` → 以 `store.append_many()` 一次 Redis pipeline 批次寫入 `monitor:stream:logs`（`transaction=False`，N 筆 → 1 round-trip）；**✅ 已實作**：每次 flush 同時計算 `minid = now_ms - minid_seconds × 1000` 並傳入 `append_many(minid=minid)`（`minid_seconds=0` → 傳 `None`，只靠 `MAXLEN`）；失敗整批 log warning、佇列繼續（best-effort、all-or-nothing 與 best-effort 語意一致）。
...
class TimeSeriesStore(Protocol):
    async def append(self, stream: str, entry: dict, *,
                     maxlen: int | None = None, minid: int | None = None) -> str: ...
    async def append_many(self, stream: str, entries: list[dict], *,
                          maxlen: int | None = None, minid: int | None = None) -> list[str]: ...
    # RedisStreamStore 以 pipeline(transaction=False) 實作：N 筆 XADD → 1 round-trip
    # approximate = maxlen is not None or minid is not None（自動啟用 ~ 近似修剪）
    async def query(self, stream: str, *, since: int | None = None, until: int | None = None,
                    cursor: str | None = None, limit: int = 100) -> "Page": ...

class RedisStreamStore:                       # interim（XADD MAXLEN ~ / MINID ~ / XRANGE）
...
async def run_log_flusher(
    ...
    batch_size: int,
    maxlen: int,
    minid_seconds: int = 0,   # 0 = 只靠 MAXLEN；> 0 → 每次 flush 帶 MINID ~ (now_ms - retention_ms)
) -> None: ...   # lifespan task
...
- ✅ `append` / `append_many` 接受 `minid` kwarg 不拋（`minid=0` → 不修剪任何筆）。
- ✅ `append_many(minid=5000)` → Stream ID < 5000 的舊筆被 `XADD MINID ~` 淘汰；ID ≥ minid 的新筆保留（以 raw FakeRedis 注入顯式舊 ID 驗證）。
...
- ✅ `run_log_flusher(minid_seconds=3600)` → `append_many` 收到 `minid ≈ now_ms - 3_600_000`（5 秒容差），以 spy 驗值；`minid_seconds=0`（預設）→ `minid=None`（只靠 MAXLEN，向後相容）。
...
- ✅ **Redis Stream 為 hot store（已定案）**，藏在 `TimeSeriesStore` 介面後；有界（`MAXLEN ~` ＋ **`MINID ~`（✅ 已實作時間保留）**）、非長期歸檔；游標＝entry ID。`MAXLEN` 從數量截斷、`MINID` 從時間截斷，兩者共存互補；`minid_seconds=0` 僅靠 `MAXLEN`（向後相容）。**未來接外部聚合器/TSDB＝換 adapter、上層零改**。
- ✅ **日誌擷取非阻塞**：`logging.Handler` → 有界佇列（滿即丟）→ 背景 flush 呼叫 `store.append_many(maxlen=..., minid=...)`（Redis pipeline `transaction=False`，N 筆 → 1 round-trip，避免串行 N 次 XADD 的吞吐量懸崖）；每次 flush 按 `monitoring_retention_seconds` 計算 `minid`，`0` → 傳 `None` 不修剪；錯誤語意為 all-or-nothing，與 best-effort 監控一致；沿用既有遮罩 filter；`APP_ENV=test`/`monitoring_enabled=False` 不啟用。
