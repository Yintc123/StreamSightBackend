...
│   ├── db/                 # DeclarativeBase（base.py）、engine + session（session.py）、加密欄位型別（types.py）
...
├── repositories/           # repo_base.py（BaseRepository）+ 各 repo（repo_admin / repo_user / repo_identity / repo_record 等）
├── services/
│   ├── service_auth.py         # AuthService（register / login / refresh / logout）
│   ├── service_admin.py        # AdminService（CRUD、生命週期、升降級、稽核）
│   ├── service_user.py         # UserService
│   ├── service_record.py       # RecordService（含批次匯入）
│   ├── service_initial_admin.py # Bootstrap root upsert（startup 時執行）
...
└── dtos/                   # 跨層 Pydantic DTO（dto_auth / dto_user / dto_record / dto_monitoring / dto_ws）
...
| `admin-account-refinement.md` | Admin 帳號欄位 / 生命週期精修 |
| `bootstrap-hidden-admin.md` | Bootstrap root 機制 |
| `enum-int.md` | IntEnum rank = value 設計 |
| `implementation-roadmap.md` | 實作路線圖 / 里程碑 |
